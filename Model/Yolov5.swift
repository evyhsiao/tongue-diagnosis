...
class Yolov5 {
    ...
    init?() {
        ...
        let paramPath = Bundle.main.path(forResource: "best", ofType: "param")
        guard net.loadParam(paramPath) == 0 else {
            return nil
        }
        let modelPath = Bundle.main.path(forResource: "best", ofType: "bin")
    ...
    func predict(for image: UIImage) -> ([Object], Double) {
        ...
        let input: Mat = Mat.init(fromImageResize: image, Int32(targetW), Int32(targetH))
        ...
        let outputs = net.run(withName: ["images": input], ["output", "798", "818"])!
        
        var proposals: [Object] = []
        proposals += generateProposals(stride: 8, input: input, feature: outputs["output"]!)
        proposals += generateProposals(stride: 16, input: input, feature: outputs["798"]!)
        proposals += generateProposals(stride: 32, input: input, feature: outputs["818"]!)
...
let labels = [
    "crenated tongue", "fissured tongue", "normal tongue"
]
